...
void main() {
  group('TimeRange', () {
    ...
    testWidgets('should have correct display names', (WidgetTester tester) async {
      await tester.pumpWidget(createTestWidget(
        child: Builder(
          builder: (context) {
            expect(TimeRange.lastMonth.displayName(context), equals('Last Month'));
            expect(TimeRange.lastThreeMonths.displayName(context), equals('Last 3 Months'));
            expect(TimeRange.lastYear.displayName(context), equals('Last Year'));
            expect(TimeRange.allTime.displayName(context), equals('All Time'));
            return Container();
          },
        ),
      ));
    ...
    test('should return correct start date for lastMonth', () {
      ...
      final expectedDate = DateTime(now.year, now.month, 1); // First day of current month
    ...
    test('should return correct start date for lastThreeMonths', () {
      final now = DateTime.now();
      final expectedDate = DateTime(now.year, now.month - 2, 1); // First day of 3 months ago
    ...
    test('should return correct start date for lastYear', () {
      final now = DateTime.now();
      final expectedDate = DateTime(now.year, now.month - 11, 1); // First day of 12 months ago
    ...
    test('should handle month overflow for lastMonth', () {
      ...
      final testDate = DateTime(januaryDate.year, januaryDate.month - 1, januaryDate.day);
    ...
    test('should handle month overflow for lastThreeMonths', () {
      ...
      final testDate = DateTime(februaryDate.year, februaryDate.month - 3, februaryDate.day);
